...
function createJoint(overrides: Partial<RobotState['joints'][string]> = {}): RobotState['joints'][string] {
  return {
    id: 'elbow_joint',
    name: 'elbow_joint',
...
test('TreeNodeJointBranchList does not highlight the joint row when only the child link is selected', async () => {
  ...
  try {
    ...
    const jointRow = container.querySelector(`[title="${joint.name} · Revolute"]`) as HTMLDivElement | null;
    assert.ok(jointRow, 'joint row should render');

    assert.ok(
      !jointRow.className.includes('bg-system-blue/10 text-text-primary ring-1 ring-inset ring-system-blue/15'),
      'child link selection should not promote the parent joint row into hovered highlight state',
    );
    assert.ok(
      !jointRow.className.includes('bg-system-blue/10 text-text-primary shadow-sm ring-1 ring-inset ring-system-blue/20'),
